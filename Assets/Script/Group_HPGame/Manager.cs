...
using GroupHPVictoryDecide;
using GroupHpPlayer;


namespace GroupHpGame 
{
    /*
        分成兩個組別 每個組別共用分數
        
        骰到1扣自己玩家血量 當血量為零 玩家不能擲骰子

        獲勝條件 : 
        1.當有一組的玩家血量都為零 敵方獲勝
        2.當有一組 先獲得 100 分  獲得100分的組別獲勝
    ...
    public class Manager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            RollZeroBtn.onClick.AddListener(delegate() 
            {
            ...
            GameOne.onClick.AddListener(delegate() 
            {
                ...
                _PlayerControl = new VictoryDecide(GroupCount,WinnerText, PlayerNames, PlayerHPimage);
            ...
            {
                int PlayersFraction = _PlayerControl.SetPlayerFraction(ref NowGroup);
                PlayersFraction_text[NowGroup].text = PlayersFraction.ToString();
        ...
        void init()
        {
            ...
            PlayerNames = new string[] {"Group1PlayerOne", "Group1PlayerTwo", "Group2PlayerOne", "Group2PlayerTwo" };
            GroupCount = 2;
