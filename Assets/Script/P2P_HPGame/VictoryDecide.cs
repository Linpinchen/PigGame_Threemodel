...
namespace P2PHPVictoryDecide 
{
    public class VictoryDecide : PlayerControl
    {
        ...
        public VictoryDecide ( Text WinnerText,string[] Names,Image[] Hpimage) 
        {
        ...
        public void VictoryDecide_FC() 
        {
            ...
            Debug.Log($" VictoryDecide_FC, NowPlayer.Enemy : {NowPlayer.Enemy.PlayerName}");
                ...
                if (NowPlayer.Fraction >= 100)
                {
                    ...
                    if (NowPlayer.Enemy.HP > 0)
                    {
                        Debug.Log($"NowPlayer 的 敵人的血量 :{NowPlayer.Enemy.HP}");

                        NowPlayer.Enemy.HP -= 1;

                        Debug.Log($"NowPlayer 的 敵人被扣血後的血量 :{NowPlayer.Enemy.HP}");


                        float temp = (((100 / 5) * 0.01f) * NowPlayer.Enemy.HP);

                        NowPlayer.Enemy.HPimage.rectTransform.localScale = new Vector3(temp, 1, 1);

                        if (NowPlayer.Enemy.HP <= 0)
                        {
                            OpenRest = true;
                            Debug.Log("NowPlayer獲勝");
